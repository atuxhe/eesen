...
#ifndef EESEN_LSTMPROJ_PARALLEL_LAYER_H_
#define EESEN_LSTMPROJ_PARALLEL_LAYER_H_
...
class LstmProjParallel : public LstmProj {
public:
    LstmProjParallel(int32 input_dim, int32 output_dim) : LstmProj(input_dim, output_dim)
    { }
    ~LstmProjParallel()
    { }

    Layer* Copy() const { return new LstmProjParallel(*this); }
    LayerType GetType() const { return l_LstmProj_Parallel; }
    ...
    void PropagateFnc(const CuMatrixBase<BaseFloat> &in, CuMatrixBase<BaseFloat> *out) {
      ...
      propagate_buf_.Resize((T+2)*S, 7 * cell_dim_+recur_dim_, kSetZero);
      ...
      for (int t = 1; t <= T; t++) {
        ...
        y_GIFO.AddMatMat(1.0, YR.RowRange((t-1)*S,S), kNoTrans, wei_gifo_r_, kTrans,  1.0);
        // input gate
        ...
        // the projection m -> r
        y_r.AddMatMat(1.0, y_m, kNoTrans, wei_r_m_, kTrans, 0.0);
...
//      } 
    ...
    void BackpropagateFnc(const CuMatrixBase<BaseFloat> &in, const CuMatrixBase<BaseFloat> &out,
      ...
      backpropagate_buf_.Resize((T+2)*S, 7 * cell_dim_+recur_dim_, kSetZero);
      ...
      for (int t = T; t >= 1; t--) {
        ...
        // d_r comes from two parts: errors from the upper layer and errors from the following frame (t+1)
        d_r.AddMatMat(1.0, DGIFO.RowRange((t+1)*S,S), kNoTrans, wei_gifo_r_, kNoTrans, 1.0);
        d_m.AddMatMat(1.0, d_r, kNoTrans, wei_r_m_, kNoTrans, 0.0);
        // d_h
...
//      }
      ...
      wei_gifo_r_corr_.AddMatMat(1.0, DGIFO.RowRange(1*S, T*S), kTrans, YR.RowRange(0*S,T*S), kNoTrans, mmt);
      ...
      wei_r_m_corr_.AddMatMat(1.0, DR.RowRange(1*S,T*S), kTrans, YM.RowRange(1*S,T*S), kNoTrans, mmt);
    }
