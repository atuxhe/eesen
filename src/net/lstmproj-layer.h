...
#ifndef EESEN_LSTMPROJ_LAYER_H_
#define EESEN_LSTMPROJ_LAYER_H_
...
class LstmProj: public TrainableLayer {
public:
    LstmProj(int32 input_dim, int32 output_dim) :
        TrainableLayer(input_dim, output_dim),
        cell_dim_(output_dim * 4),
        recur_dim_(output_dim),
    ...
    Layer* Copy() const { return new LstmProj(*this); }
    LayerType GetType() const { return l_LstmProj; }
    LayerType GetTypeNonParal() const { return l_LstmProj; }   
    ...
    void InitData(std::istream &is) {
      ...
      wei_gifo_r_.Resize(4 * cell_dim_, recur_dim_);  wei_gifo_r_.InitRandUniform(param_range);
      wei_r_m_.Resize(recur_dim_, cell_dim_);  wei_r_m_.InitRandUniform(param_range);
    ...
    void ReadData(std::istream &is, bool binary) {
      ...
      wei_gifo_r_.Read(is, binary);
      wei_r_m_.Read(is, binary);
      ...
      wei_gifo_r_corr_ = wei_gifo_r_; wei_gifo_r_corr_.SetZero();
      wei_r_m_corr_ = wei_r_m_; wei_r_m_corr_.SetZero();
    ...
    void WriteData(std::ostream &os, bool binary) const {
      ...
      wei_gifo_r_.Write(os, binary);
      wei_r_m_.Write(os, binary);
    ...
    std::string Info() const {
        return std::string("    ") + 
            ...
            "\n  wei_gifo_r_  "   + MomentStatistics(wei_gifo_r_) +
            "\n  wei_r_m_  "   + MomentStatistics(wei_r_m_) +
    ...
    std::string InfoGradient() const {
        return std::string("    ") +
            ...
            "\n  wei_gifo_r_corr_  "   + MomentStatistics(wei_gifo_r_corr_) +
            "\n  wei_r_m_corr_  "   + MomentStatistics(wei_r_m_corr_) +
    ...
    void PropagateFnc(const CuMatrixBase<BaseFloat> &in, CuMatrixBase<BaseFloat> *out) {
        ...
        propagate_buf_.Resize(T + 2, 7 * cell_dim_+recur_dim_, kSetZero);
        ...
        CuSubMatrix<BaseFloat> YR(propagate_buf_.ColRange(7 * cell_dim_, recur_dim_));
        ...
        for (int t = 1; t <= T; t++) {
          ...
          y_gifo.AddMatVec(1.0, wei_gifo_r_, kNoTrans, YR.Row(t-1), 1.0);
          // input gate
          ...
          y_r.AddMatVec(1.0,wei_r_m_,kNoTrans,y_m, 0.0);
        }  // end of loop t
    ...
    void BackpropagateFnc(const CuMatrixBase<BaseFloat> &in, const CuMatrixBase<BaseFloat> &out,
        ...
        backpropagate_buf_.Resize(T + 2, 7 * cell_dim_+recur_dim_, kSetZero);
        ...
        CuSubMatrix<BaseFloat> DR(backpropagate_buf_.ColRange(7 * cell_dim_, recur_dim_));
        ...
        for (int t = T; t >= 1; t--) {
          ...
          d_m.AddMatVec(1.0, wei_r_m_, kNoTrans, d_r, 0.0);
        ...
        wei_gifo_r_corr_.AddMatMat(1.0, DGIFO.RowRange(1,T), kTrans, YR.RowRange(0,T), kNoTrans, mmt);
        ...
        wei_r_m_corr_.AddMatMat(1.0, DR.RowRange(1,T), kTrans, YM.RowRange(1,T), kNoTrans, mmt);
    }
    ...
    void Update(const CuMatrixBase<BaseFloat> &input, const CuMatrixBase<BaseFloat> &diff) {
      ...
      if (max_grad_ > 0) {
        ...
        wei_gifo_r_corr_.ApplyFloor(-max_grad_); wei_gifo_r_corr_.ApplyCeiling(max_grad_);
        wei_r_m_corr_.ApplyFloor(-max_grad_); wei_r_m_corr_.ApplyCeiling(max_grad_);
      ...
      wei_gifo_r_.AddMat(-lr, wei_gifo_r_corr_);
      wei_r_m_.AddMat(-lr, wei_r_m_corr_);
    ...
    void Scale(BaseFloat scale) {
      ...
      wei_gifo_r_.Scale(scale);
      wei_r_m_.Scale(scale);
      bias_.Scale(scale);
    ...
    void Add(BaseFloat scale, const TrainableLayer & layer_other) {
      const LstmProj *other = dynamic_cast<const LstmProj*>(&layer_other);
      wei_gifo_x_.AddMat(scale, other->wei_gifo_x_);
      wei_gifo_r_.AddMat(scale, other->wei_gifo_r_);
      wei_r_m_.AddMat(scale, other->wei_r_m_);
    ...
    int32 NumParams() const {
      return wei_gifo_x_.NumRows() * wei_gifo_x_.NumCols() +
             wei_gifo_r_.NumRows() * wei_gifo_r_.NumCols() +
             wei_r_m_.NumRows() * wei_r_m_.NumCols() +
             bias_.Dim() +
             phole_i_c_.Dim() +
             phole_f_c_.Dim() +
             phole_o_c_.Dim();
    ...
    void GetParams(Vector<BaseFloat>* wei_copy) const {
      ...
      size = wei_gifo_r_.NumRows() * wei_gifo_r_.NumCols();
      wei_copy->Range(offset, size).CopyRowsFromMat(wei_gifo_r_); offset += size;
      size = wei_r_m_.NumRows() * wei_r_m_.NumCols();
      wei_copy->Range(offset, size).CopyRowsFromMat(wei_r_m_); offset += size;
      size = bias_.Dim();
...
protected:
    ...
    CuMatrix<BaseFloat> wei_gifo_r_;
    CuMatrix<BaseFloat> wei_r_m_; //projection layer r: from m to r
    ...
    CuMatrix<BaseFloat> wei_gifo_r_corr_;
    CuMatrix<BaseFloat> wei_r_m_corr_; //projection layer r: from m to r
