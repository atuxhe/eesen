...
template<typename Real>
void MatrixBase<Real>::AddMatDotMat(const Real alpha,
                                    const MatrixBase<Real>& A, MatrixTransposeType transA,
                                    const MatrixBase<Real>& B, MatrixTransposeType transB,
                                    const Real beta) {
    // fow now kTrans in not supported
    KALDI_ASSERT(transA == kNoTrans);
    KALDI_ASSERT(transB == kNoTrans);
    KALDI_ASSERT(A.NumRows() == B.NumRows() && A.NumCols() == B.NumCols());
...
void MatrixBase<Real>::DiffRelu(const MatrixBase<Real> &value,
  ...
  for (MatrixIndexT r = 0; r < num_rows; r++) {
    for (MatrixIndexT c = 0; c < num_cols; c++)
      data[c] = (value_data[c] > 0.0) * diff_data[c];
    data += stride;
    value_data += value_stride;
    diff_data += diff_stride;
  }
