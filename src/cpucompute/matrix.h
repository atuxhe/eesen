...
#ifndef CPUCOMPUTE_MATRIX_H_
#define CPUCOMPUTE_MATRIX_H_ 1
...
class MatrixBase {
 public:
  ...
  // friend declarations for CUDA matrices (see ../gpucompute/)
  ...
  /// Copy from CUDA matrix.  Implemented in ../gpucompute/cuda-matrix.h
  template<typename OtherReal>  
  ...
  /// This version of CopyRowsFromVec is implemented in ../gpucompute/cuda-vector.cc
  ...
  // <jiayu>
  // *this = alpha * M[^T] * diag(v) + beta * *this
  void AddMatDiagVec(const Real alpha, 
                     const MatrixBase<Real> &M, MatrixTransposeType transM, 
                     VectorBase<Real> &v,
                     Real beta = 1.0);

  // dot product, element by element, *this = alpha * a * b + beta * *this;
...
class Matrix : public MatrixBase<Real> {
 public:
  ...
  /// This is defined in ../gpucompute/cuda-matrix.h
  template<typename OtherReal>
  ...
  /// Defined in ../gpucompute/cuda-matrix.cc
